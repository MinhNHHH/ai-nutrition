...
def load_env():
  """
  Loading all environments variable that has a DC_ prefix and strip it.
  DC_QUEUE_PUSH_NAME => QUEUE_PUSH_NAME
  To get this env, call `get_default("QUEUE_PUSH_NAME")`
  """
  ...
  for env in os.environ:
    if env.startswith("APP_"):
      env_name = env.removeprefix("APP_")
